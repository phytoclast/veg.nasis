...
y = rnorm(20,5,2)^2
n=3
df <- data.frame(x=crown2$x, y=crown2$y)|> unique()
df <- df |> mutate(x=floor(x*1000)/1000, y=floor(y*1000)/1000) |> unique()
df <- data.frame(x=x,y=y)
...
df3 <- df[ret,]
ggplot()+
  geom_polygon(data=df2, aes(x=x, y=y), color='blue',fill='#00009960')+
  geom_polygon(data=df3, aes(x=x, y=y), color='green',fill='#00990050')+
  geom_point(data=df, aes(x=x, y=y), color='red',fill='#99000050')+
...
#trial 4 - redo convex hull ----

x = c(-1,-1.5,0,1,1.2,3,1,0.5)
y = c(0,1,0.5,1,1,0.5,0,0.2)
df <- data.frame(x=x,y=y)
df <- df |> mutate(q = 1:nrow(df))
rm(x,y)
...
amin = min(subset(df, !s %in% 1)$a1)
df <- df |> mutate(s = ifelse(a1 == amin & is.na(s), 0, s))
for(i in 1:nrow(df)){#nrow(df) i=8
  if(check){
    x0 <- df[df$s %in% (i-1),]$x
    y0 <- df[df$s %in% (i-1),]$y
    x1 <- df[df$s %in% i,]$x
