...
crown2 <- vegnasis::cavhull(x=crown$x,y=crown$y, concave = F)
 ...
 opposite=F
 oppfactor = ifelse(opposite,1,2)
 n = pmax(floor(5*(ht.max-ht.min)*(bu-bl)/10*oppfactor/crwd*5),1)
 crshape = c('dome')
 ...
 cstem <- stem
 branch <- makeStem(crwd*0.05*ca, dbh/100*0.1,0.01,10)#small branch near top
 stem <- attachBranch(stem, branch, -30, ht.max*0.9)
 for(i in 1:nrow(shapes)){#i=1
   branch <- makeStem(shapes$l[i], shapes$d[i]*0.5,0.01,10)
   bpos <- max(branch$y)
   branch2 <- branch |> mutate(x=x*0.2*bf*ca,y=y*0.2*bf)
   # branch2 <- skewStem(branch2, amp=ifelse(shapes$a[i] >= 0,-0.07*(shapes$s[i]*-1+1),0.07*(shapes$s[i]*-1+1)), phase=0, waves=2)
   branch3 <- branch |> mutate(x=x*0.1*bf*ca,y=y*0.1*bf)
   branchx1 <- branch |> mutate(x=x*0.3*bf,y=y*0.3*bf)
