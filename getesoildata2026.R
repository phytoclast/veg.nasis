...
for(i in 1:length(soilstates)){#i=17
  ...
  if(is.null(soilall)){
    soilall <- soil
    soilsummaryall <- soilsummary
  }else{
    soilall <- rbind(soilall,soil)
    soilsummaryall <- rbind(soilsummaryall,soilsummary)
  }
}
...
valclass <- soilsummaryall[,c('mukey','sand150')] |> as.matrix()
r <- classify(mkeys, valclass, others=NA)
names(r) <- 'sand150'
writeRaster(r, 'gdbtemp/sand150.tif', overwrite=T)
